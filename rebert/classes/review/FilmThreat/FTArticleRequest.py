...
#
#   March 2026 - update to rating extraction because of change to the formatting
#
#{{RELEASE}}
#
#{{COPYRIGHT}}
...
#####
#   
#   START class FTArticleRequest definition
#   
#####

###
#   A class/object that interacts with Film Threat website to collect 
#   review articles.
#
class FTArticleRequest(ReviewArticleBase):
    '''
    The FTArticleRequest class is a subclass of ReviewArticleBase that connects to the
    Film Threat website to request a specified article. This is assumed to be a review article, 
    ...
    '''
    def __init__(self, name="FTArticleRequest", logger=None, *args, **kwargs):
        '''
    ...
    def _parseHTMLPage_(self, text=None, review=None):
        ...
        review_type = "movie_review"    # the item being reviewed
        #
        #   Now, parse the text of the page
        html_parse = BeautifulSoup(text,'html.parser')
        #   The review is contained within a 'main' tag
        main = html_parse.find('main')
        #   In the 'main' we need to find a specific div that is class 'article'
        article = ""
        divs = main.find_all('div')
        for div in divs:
            if 'class' in div.attrs and 'article' in div['class']:
                article = div
                break
        #   A little check
        if not article: 
            self.log(f"When processing page {review['review_url']}", level="WARNING")
            self.log(f"Could not extract 'article' div from main body content", level="WARNING")
            return review
        #
        #   In the 'article' div we want to extract three other divs
        author_div = ""
        content_div = ""
        summary_div = ""
        ...
        for div in divs:
            if 'class' in div.attrs:
                if 'author' in div['class'] and not author_div:
                    author_div = div
                if 'content-editor' in div['class'] and not content_div:
                    content_div = div
                if 'movie-summary' in div['class'] and not summary_div:
                    summary_div = div
            if author_div and content_div and summary_div: break
        #
        #   Ok, should now have the div chunks to parse out the pieces
        #   Several items are extracted from the author_div
        if author_div:
            review_title = self._extractArticleTitle_(author_div)
            title = self._extractMovieTitleFromArticleTitle_(review_title)
            author = self._extractAuthor_(author_div)
            post_date = self._extractPostDate_(author_div)
        #
        #
        if content_div:
            body = self._extractContent_(content_div)
        if summary_div:
            rating = self._extractRating_(summary_div)
        #
        #print(f"{review_title=}")
        #print(f"{title=}")
        #print(f"{author=}")
        #print(f"{str(post_date)=}")
        #print(f"{body=}")
        #print(f"{rating=}")
        #
        #
    ...
    def _extractRating_(self, blob_div=None):
        ...
        for div in divs:
            if 'class' in div.attrs and 'summary-rating' in div['class']:
                #print(div)
                rating_str = div.p.text.partition('/')[0].strip()
                rating_str = rating_str.split()[-1]
                #print(rating_str)
                try:
                    star_count = int(rating_str)
                except:
                    try:
                        star_count = float(rating_str)
                    except:
                        start_count = -1
                break
        #
        ...
        if isinstance(star_count,int):
            rating_str = f"{star_count} out of 10"
        else:
            rating_str = f"{star_count:0.1f} out of 10"
    ...
    def _extractContent_(self, blob_div=None):
        ...
        '''
        #   This site will "page" reviews, so first we look for a 'pagination'
        #   div element - that should have an <a> href item that we can use to
        #   get the "next" page
        divs = blob_div.find_all('div')
        add_review = None
        for div in divs:
            if 'class' in div.attrs and 'pagination' in div['class']:
                url = div.a['href']
                anchor_text = div.text.strip().lower()
                if anchor_text == "continue":
                    requester = FTArticleRequest(name="Nested_FTArticle_Request",
                                                 logger=self.getLogger())
                    #print(f"Requesting: {url}")
                    add_review = requester.getReviewArticle(url=url)
                break
        
        callouts = list()
        co_elts = blob_div.find_all("blockquote")
        for co in co_elts:
            cot = co.text
            callouts.append(cot)
        ...
        for para in paragraphs:
            #   Some paragraphs in the body are not callouts - and not body text
            #   This checks to see if we found a paragraph element that happens to
            #   be a image/figure caption - if so, skip that
            if 'class' in para.attrs and 'wp-caption-text' in para['class']:
                continue
            para_text = para.text.strip()
            if para_text and (para_text not in callouts):
                body = body+"\n"+para_text
        body = body.strip()
        #
        #   Above, we tried to collect the page of review text that
        #   follows this page ... if it was there, add it on
        if add_review:
            body = body+"\n"+add_review['review']
        body = body.strip()
        return body
    ...
    def _extractPostDate_(self, blob_div=None):
        ...
        post_date = ""
        post_date_ts = ""
        time_elt = blob_div.p.text.strip()
        try:
            #   The text is a nice formatted string
            post_date = time_elt.rpartition("|")[2].strip()
        except:
            pass
...
#####
#   
#   END class FTArticleRequest definition
#   
#####

if __name__ == '__main__':
    print("FTArticleRequest.py is a class with no main()")
