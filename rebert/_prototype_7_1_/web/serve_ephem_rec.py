...
#
#
def render_mainpage(ui_state, session_state):
    ui_state = apply_explanation_ui_state(ui_state)
    return render_template("mainpage.html", state=ui_state, session=session_state)
#
#
#
#
...
def serve_next_ephem_question(request, server_state):
    ...
    if not state:
        ...
        page = render_mainpage(ui_state, session_state)
    ...
    return page
#
#   This collects the state information from the server and then
#   calls the make_ephem_rec_request() function to actually make
#   the request to the LLM that classifies the movies into 
...
def get_discussion_content_list(session_state):
    ...
    for i in range(0,6):
        ...
        if session_state['rec_flags'] and session_state['rec_flags'][i]['rebert_rationale']:
            ...
            if REBERT_GROUNDED_EXPLANATIONS and session_state['rec_flags'][i].get('rebert_explanation'):
                turn = f"Based on your answers, I predict this movie is {rating} with your interests.\n\n"
                turn = turn + "See the \"Why this title\" panel above for how your answers and reviews connect to this pick.\n\n"
...
def update_recommendation_flags(session_state):
    ...
    if session_state['highlights'] and rec_list:
        for movie in session_state['highlights']:
            ...
            if movie.get("rebert_explanation"):
                rec_list[column]["rebert_explanation"] = movie["rebert_explanation"]
            #
            ...
            if 'highly' in rebert_rating:
                ...
                if rec_list[column]['rating'] >= 3.5:
                    ...
                    # Low score, mild match make the 'meh' recommendation
                    #rec_list[column]['flag'] = ""
                    #rec_list[column]['text'] = ""
                    rec_list[column]['flag'] = "rec-meh-small.png"
                    rec_list[column]['text'] = "recommendation: meh"
            elif 'working' in rebert_rating:
                rec_list[column]['flag'] = "rec-working-small.png"
                rec_list[column]['text'] = "recommendation: no recommendation, still working"
                pass
