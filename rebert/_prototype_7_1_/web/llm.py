...
#
#
def build_ephem_recommendation_user_question(movie_title=""):
    question = EPHEM_RECOMMENDATION_QUESTION.format(movie_title=movie_title)
    if REBERT_GROUNDED_EXPLANATIONS:
        question = question + EPHEM_RECOMMENDATION_EXPLANATION_EXTENSION
        if REBERT_REVIEW_EXCERPT_IDS:
            question = question + EPHEM_RECOMMENDATION_EXCERPT_EXTENSION
    return question
#
#
def create_ephem_user_description(session=None):
    if REBERT_GROUNDED_EXPLANATIONS:
        return create_ephem_qna_str_numbered(session)
...
#
#
def create_ephem_review_context(reviews=None):
    if REBERT_REVIEW_EXCERPT_IDS:
        return create_review_excerpt_str(reviews)
    return create_movie_review_str(reviews)
#
#
...
def make_ephem_rec_request(session=None, chat_key=""):
    ...
    user_description = create_ephem_user_description(session)
    #
    ...
    for movie in session["highlights"]:
        ...
        review_str = create_ephem_review_context(reviews)
        if user_description and review_str:
            ...
            user_turn = ChatMessage()
            user_turn.setRole("user")
            uprompt = build_ephem_recommendation_user_question(movie_title=movie["title"])
    ...
    for ephem_thread in threaded_requests:
        ...
        if ephem_thread['requester']:
            ...
            try:
                ...
                if REBERT_GROUNDED_EXPLANATIONS:
                    explanation = parse_structured_explanation(content)
                    ephem_thread["movie"]["rebert_explanation"] = explanation
                    tooltip_text = format_explanation_for_tooltip(explanation)
                    if tooltip_text:
                        ephem_thread["movie"]["rebert_rationale"] = tooltip_text
                else:
                    ephem_thread["movie"]["rebert_explanation"] = None
            except:
                ...
                ephem_thread["movie"]["rebert_explanation"] = empty_explanation()
