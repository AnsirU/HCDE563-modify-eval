...
import os
#
#   Study / evaluation mode for grounded explanations (HCDE 563 project).
#   Set REBERT_EXPLANATION_MODE before launching the server:
#     baseline  — original p7.1 recommendation UI (no structured explanation panel)
#     modified  — structured user-linked explanations (default for this fork)
#     modified_full — modified + review excerpt IDs passed to the LLM (Level 3)
#
_explanation_mode = os.environ.get("REBERT_EXPLANATION_MODE", "modified").strip().lower()
REBERT_EXPLANATION_MODE = _explanation_mode
REBERT_GROUNDED_EXPLANATIONS = _explanation_mode in ("modified", "modified_full", "explain", "on", "true", "1")
REBERT_REVIEW_EXCERPT_IDS = _explanation_mode == "modified_full" or os.environ.get(
    "REBERT_REVIEW_EXCERPT_IDS", ""
...
#
#
#   This sets where the user data will live. This is currently in the
#   server directory. That is probably ok for a prototype. In a production
#   system, we would probably want to use a directory outside of the
#   server space.
#
#   Of course, if this were a production system - we would also use a real
#   database - MongoDB or MySQL or ... -- rather than this data directory
#   system that was built for rebert.
#
REBERT_PROFILES_DIRECTORY = 'web/data'
REBERT_PROFILES_FOLDER = 'rebert_users'
#
#   Basically the same for a user sessions database
#
REBERT_SESSIONS_DIRECTORY = 'web/data'
REBERT_SESSIONS_FOLDER = 'rebert_sessions'
#
#   Basically the same for a user ratings database
#
REBERT_RATINGS_DIRECTORY = 'web/data'
REBERT_RATINGS_FOLDER = 'rebert_user_ratings'
#
#
...
#   Added prototype 6
    ...
    "rebert_explanation": None      # structured explanation dict when enabled
}
#
...
REBERT_MAINPAGE_UI_STATE_TEMPLATE = {
    ...
    "show_explanations": False,     # show the "Why this title" panel
    "explanation_mode_label": ""   # baseline vs modified banner text
}
...
#
REBERT_THREADED_EPHEM_REC_TEMPLATE = {
...
#
#   Account Login related constants used when creating an account or when the user
#   is attempting to login with their credentials
#   
REBERT_CREATE_ERROR_INFO_NEEDED = "You must supply a username and email to create an account. Set a username and email address, and then try account creation again."
#
REBERT_CREATE_ERROR_UNAME = "The username you supplied is already being used by another user. Change your username and try account creation again."
REBERT_CREATE_ERROR_EMAIL = "The email address you supplied is already being used by another user. Change your email address and try account creation again."
REBERT_CREATE_ERROR_PASS = "The passwords that you supplied did not match. Make sure your passwords match and then try account creation again."
REBERT_CREATE_ERROR_DB = "Something went wrong during the creation of your account. Contact account services."
#
#   The error message to present when the login fails.
REBERT_LOGIN_ERROR_MESSAGE = "The username, user email, or password that you provided does not appear to match any records. Check your credentials and try again."
#
#   The set of characters that are allowed in a user name. This is very conservative, just
#   alphanumeric plus a few punctuation.
REBERT_ALLOWED_USERNAME_CHARS = "1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz.-_"
...
#   This actually generates N+1 questions, but the last question
#   could be ignored by the user
REBERT_MINIMUM_RATING_QNA = 2
#
#   The minimum number of words before an answer is accepted
#REBERT_MINIMUM_ANSWER_WORDS = 15
REBERT_MINIMUM_ANSWER_WORDS = 10
#
#   The minimum length of answer in words - if the length is not met
#   then rebert asks another question about the movie
REBERT_MINIMUM_RATING_WORDS = 125
...
REBERT_RATEPAGE_UI_STATE_TEMPLATE = {
    ...
    "edit_view_date":       "",         # last time they remember watching this film
...
#
#   This is a Q&A feedback record - eventually stored in a DB
#
REBERT_MOVIE_FEEDBACK_QNA = {
    'rtype':                    'q&a',      #   A 'record type' for mixed DB search
    'account_id':               '',         #   The user account_id
