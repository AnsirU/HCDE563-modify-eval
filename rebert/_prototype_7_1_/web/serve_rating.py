...
from rebert._prototype_7_1_.web.prompts import *
from rebert._prototype_7_1_.web.llm import *
from rebert._prototype_7_1_.web.movies import *
...
##############
#
#   Basic audio support on MacOS browsers
#       Chrome: mp4, webm
#       Opera: mp4, webm
#       Safari: mp4
#       Firefox: ogg, webm
...
##############
#
#   Turn off test data loading to use the database
#   
REBERT_LOAD_RATED_MOVIES_TEST = False
...
def serve_rating_page(request, server_state):
    ...
    session_state = load_session_state(str(session_id))
    if not session_state:
        print_server_log(f"Could not load session state with session_id {str(session_id)}",
                         "serve_rating_page()",
                         MODULE_RATING_DEBUG)
    #
    ...
    if REBERT_LOAD_RATED_MOVIES_TEST:
        try:
            ...
            print_server_log(f"Could not load test ratings from: {REBERT_RATED_MOVIES_TEST_FNAME}",
                              ...
                              MODULE_RATING_DEBUG)
    #
    #   Quick check of the cookies - part of the login process - is the user logged in?
    if request.cookies:
        print_server_log(f"session cookie: {request.cookies['session']}",
                          "serve_rating_page()",
                          MODULE_RATING_DEBUG)
    else:
        print_server_log(f"NO session cookies!",
                          "serve_rating_page()",
                          MODULE_RATING_DEBUG)
    #
    #   This will go into the session - eventually we only do this search once.
    #
    account_id = flask_login.current_user.account_id
    ratings = get_user_ratings(account_id)
    if ratings:
        print_server_log(f"Found {len(ratings)} ratings in ratings DB.",
                            "serve_rating_page()",
                            MODULE_RATING_DEBUG)
        session_state['rating']['rated'].extend(ratings)
...
def serve_rating_next(request, server_state):
    ...
    if qna['answer'] and (len(qna['answer'].split()) >= REBERT_MINIMUM_ANSWER_WORDS):
        ...
        print_server_log(f"Answer length was too short. Only {len(qna['answer'].split())} words.",
                           "serve_rating_next()",
                            MODULE_RATING_DEBUG)
    #
    ...
    if ((answer_count > REBERT_MINIMUM_RATING_QNA) and
        (answer_length > REBERT_MINIMUM_RATING_WORDS)) or ui_state['complete']:
    ...
    if ((answer_count <= REBERT_MINIMUM_RATING_QNA) or
        (answer_length <= REBERT_MINIMUM_RATING_WORDS)):
        qna_str = create_rating_qna_str(session_state)
        #
        #   User our question requester to get the next question
...
def serve_rating_edit_rating(request, server_state):
    ...
    session_state['ui_state'] = ui_state
    #
    #   Quick check of the cookies - part of the login process
    if request.cookies:
        print_server_log(f"session cookie: {request.cookies['session']}",
                          "serve_rating_edit_rating()",
                          MODULE_RATING_DEBUG)
    ...
    if ui_state['edit_qna']:
        ...
        movie['creation_ts'] = str(datetime.datetime.now()).partition('.')[0]
    ...
    if ui_state['edit_score'] != movie['score']:
        ...
        print_server_log(f"Updating the movie score.",
                           ...
                           MODULE_RATING_DEBUG)
    #
    #   If we collected recent viewing date
    if ui_state['edit_view_date'] != movie['last_viewed']:
        movie['last_viewed'] = ui_state['edit_view_date']
        movie['creation_ts'] = str(datetime.datetime.now()).partition('.')[0]
        print_server_log(f"Updating the movie last viewed date.",
                           "serve_rating_edit_rating()",
                           MODULE_RATING_DEBUG)
    #
    ...
    #   The split on a space will ruin the two word genres e.g. "Science Fiction"
    #genre_list = [str(x) for x in ui_state['edit_genres'].replace(',',' ').replace(';',' ').split()]
    genre_list = [str(x).strip() for x in ui_state['edit_genres'].split(',')]
    #
    ...
    if matched > 0:
        ...
        movie['candidates'][0] = REBERT_MOVIE_CANDIDATE_TEMPLATE.copy()
    
    #
    #   Update the info on this movie in the rating DB
    update_movie_rating(movie)
    #
    #   Remember to save the current session state
    save_session_state(session_state)    
    #
    #   Collected all we need - go back to the start rating page
    page = render_template("rate_start.html",
                            session = session_state)
    return page
...
def save_rated_movie(ui_state, session_state):
    ...
    rated_movie = session_state['rating']['in_progress']
    rated_movie['creation_ts'] = str(current_time).partition('.')[0]
    #
    #   This way we don't have to keep querying the DB
    session_state['rating']['rated'].append(rated_movie)
    #
    #   Add rated movie to the RATINGS DBS
    rating_id = generate_rating_id(rated_movie)
    account_id = flask_login.current_user.account_id
    rated_movie['account_id'] = account_id
    rated_movie['rating_id'] = rating_id
    qna = list()
    #   Convert the simple Q & A pair to something that can go in the DB
    for pair in rated_movie['qna']:
        new_pair = REBERT_MOVIE_FEEDBACK_QNA.copy()
        new_pair['account_id'] = account_id
        new_pair['rating_id'] = rating_id
        new_pair['question'] = pair['question']
        new_pair['answer'] = pair['answer']
        qna.append(new_pair)
    rated_movie['qna'] = qna
    try:
        #db_log.log(f">>> Appending new rating '{rate_movie['rating_id']}'")
        user_ratings.append(rated_movie)
        user_ratings.save(REBERT_RATINGS_FILENAME,compact=False)
        #db_log.log(f">>> Finished adding rating '{rate_movie['rating_id']}'")
...
def identify_movie_being_rated(session_state, ui_state, server_state):
    ...
    new_title = ui_state['rate_title']
    print_server_log(f"Got '{new_title}' from the UI confirmation field",
                      "identify_movie_being_rated()",
                      MODULE_RATING_DEBUG)
    #
    #   Check to see if that title is different
    if new_title.lower() == session_state['rating']['in_progress']['title'].lower(): return
    #
    #   Make the search
...
def serve_transcribe_rating_audio(request, server_state):
    ...
    if response:
        ...
        transcription = dict()
        #   Get the json and then text of the transcription
        rjson = response.json()
