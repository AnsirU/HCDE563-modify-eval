...
//  ===============
//
//  GROUNDED EXPLANATIONS (HCDE 563)
//  Click an explanation bullet to highlight the linked ephemeral Q&A answer
//
//  ===============

function clearExplanationHighlights() {
    $(".explain-bullet").removeClass("active");
}

function highlightEphemAnswer(element, qIndex) {
    if (!qIndex) {
        return;
    }
    ...
    var question = $("#ephem_q_" + qIndex).val();
    var answer = $("#ephem_a_" + qIndex).val();
    if (!answer) {
        return;
    }

    $("#ephem-qna-label").text("Your answer to question " + qIndex);
    $("#ephem-qna-question").text(question || "");
    $("#ephem-qna-answer").text(answer);
    $("#ephem-qna-highlight").addClass("visible");

    $("html, body").animate({
        scrollTop: $("#ephem-qna-highlight").offset().top - 80
    }, 250);
}

function setupExplanationLinks() {
    if ($(".why-panel").length === 0) {
        return;
    }
    clearExplanationHighlights();
}
