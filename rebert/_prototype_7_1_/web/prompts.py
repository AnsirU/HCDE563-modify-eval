...
#
#   Structured explanation extension (Level 1) — appended when grounded explanations are enabled
#
EPHEM_RECOMMENDATION_EXPLANATION_EXTENSION = '''
Also provide a structured explanation using exactly these sections:

<USER_ALIGNMENT>
Provide 1 or 2 bullet points that map this recommendation to specific things the user said in Q1, Q2, or Q3. Start each bullet with Q1:, Q2:, or Q3: followed by a paraphrased alignment point. Do not quote the user verbatim.
</USER_ALIGNMENT>

<REVIEW_BASIS>
Provide 1 or 2 bullet points grounded in review themes such as tone, pacing, mood, or performance. If review evidence is weak or missing, provide exactly one bullet: "uncertain — limited review evidence available." Do not invent critic quotes, scores, or plot details that are not supported by the reviews.
</REVIEW_BASIS>

<CONFIDENCE>
State one of: high, uncertain, or generic match — followed by a brief reason (one short phrase). Use the format: high — reason here
</CONFIDENCE>
'''

#
#   Level 3 optional extension when review excerpts with IDs are supplied
#
EPHEM_RECOMMENDATION_EXCERPT_EXTENSION = '''
Review excerpts are labeled with EXCERPT_ID values (R1, R2, R3). When a review-basis bullet is supported by a specific excerpt, prefix that bullet with the excerpt id, for example: R1: pacing is deliberately slow.
'''
#
