...
def format_release_date(release_date_str="", reference_date=None):
    #
    #   Format a YYYY-MM-DD release date for display on movie cards.
    #   Past dates read like "Opened on January 12, 2025".
    #   Future dates read like "Opening in 3 days!".
    #
    ...
    if release_date < reference_date:
        month_day = f"{release_date.strftime('%B')} {release_date.day}, {release_date.year}"
        return f"Opened on {month_day}"
    #
    days_until = (release_date - reference_date).days
    if days_until == 0:
        return "Opening today!"
    if days_until == 1:
        return "Opening in 1 day!"
    return f"Opening in {days_until} days!"
